...
const UserController = {
  ...
  createUser: async (req, res) => {
    ...
    try {
      ...
      const newUser = new User({ username, email, address, password, telephone, role });
      await newUser.save();
  ...
  createORG: async (req, res) => {
    ...
    try {
      ...
      if (existingUser) return res.status(400).json({ error: 'Organization already exists' });

      const newUser = new User({ username, email, address, password, telephone, org_description, role });
      await newUser.save();
  ...
  loginUser: async (req, res) => {
    ...
    try {
      ...
      if (user.isBanned) return res.status(403).json({ error: 'User is banned. Cannot login' });
  ...
  loginAdmin: async (req, res) => {
    ...
    try {
      ...
      if (user.role !== 'admin') throw new Error('Access denied. Only admin users allowed.');
      if (user.isBanned) throw new Error('Admin user is banned. Cannot login.');
  ...
  sendActivationCode: async (req, res) => {
    try {
      ...
      res.status(200).json({ email, resetCode });
    } catch (error) {
  ...
  verifyCode: async (req, res) => {
    ...
    res.status(200).json({ message: resetCode === user.resetCode ? 'true' : 'false' });
  },
